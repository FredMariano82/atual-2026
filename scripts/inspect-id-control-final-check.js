// 🕵️ Script de Inspeção de Usuário ID Control (Versão Final Refinada)
// Executar: node scripts/inspect-id-control-final-check.js
...
async function inspectWithToken() {
    ...
    try {
        ...
        if (resp.ok) {
            ...
            console.log(`Registration (Matrícula?): ${data.registration}`);
            console.log(`BarCode: ${data.barCode}`);
            console.log(`BeginDate: ${data.beginDate} (Tipo: ${typeof data.beginDate})`);
            console.log(`EndDate: ${data.endDate} (Tipo: ${typeof data.endDate})`);
            console.log(`CPF: ${data.cpf || 'N/A'}`);
            console.log(`RG: ${data.rg || 'N/A'}`);
            console.log("=============================================");
        } else {
